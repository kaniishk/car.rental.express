...
module.exports = {
    index: (req, res) => {
        ...
        Car.find({}).where('isRented').equals(false).sort({ price: 1 }).skip(page * 5).limit(5).then(allCar => {
    ...
    searchingDate: (req, res) => {
        let searchingDate = req.query.searchDate
        let searchedCars = []
        
        Car.find({}).where('isRented').equals(false).then(allCars => {
            for (let car of allCars) {
                    searchedCars.push(car)
            }

            res.render('query/searchingDate', { searchedCars })
        })
    },
